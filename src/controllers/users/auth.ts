...
const logInHandler = async (req: Request, res: Response) => {
  console.log("In the login handler....... see header ", req.header);
...
const createAccountHandler = async (req: Request, res: Response) => {
  console.log("hosttttttt............................", req.header);
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "An error occured trying to create acount" });
  }
};
...
const passwordRecoveryCheckUser = async (req: Request, res: Response) => {
  try {
    ...
    if (!exist[0])
      return res
        .status(404)
        .json({ message: "user with credentials not found." });
    ...
    res
      .status(500)
      .json({ message: "An error occured trying to find account" });
  }
};
...
const passwordRecoveryConfirmOtp = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "An error occured tryng get user password" });
  }
};
