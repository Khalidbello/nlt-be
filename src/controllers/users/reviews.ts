...
} from "../../services/users/reviews-queries";
import {
  queryUserDp,
  queryUserProfile,
...
const reviewSubmitted = async (req: Request, res: Response) => {
  try {
    ...
    if (!courseName || !review || !courseId)
      return res
        .status(400)
        .json({ message: "Incomplete data sent to server for processing." });

    const profieData = await queryUserProfile(userId);

    const saved = await querySaveReview(
      userId,
      courseName,
      review,
      profieData.first_name
    );
...
export { getReviews, getUserDpforReview, reviewSubmitted };
