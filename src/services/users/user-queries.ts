...
import { queryNewEnrollment, updateLastVisited } from "../users/user-query-3";
...
const createNewUser = async (
  ...
  return new Promise<any>((resolve, reject) => {
    const query =
      "INSERT INTO users (first_name, last_name, email, password, phone_number, gender, joined, email_verified) VALUES (?, ?, ?, ?, ?, ?, ?, false)";

    pool.query(
      query,
      [firstName, lastName, email, password, phoneNumber, gender, joined],
      (err, result) => {
        if (err) {
          reject(err);
        } else {
...
const queryCourse = async (courseId: number): Promise<courseType> => {
  return new Promise<courseType>((resolve, reject) => {
    const query =
      "SELECT course_name, course_id, image, course_title, course_description, created_at, price, full_price_discount, status FROM courses WHERE course_id = ?";
...
const queryRecentcourse = async (userId: number): Promise<enrolledType> => {
  return new Promise<enrolledType>((resolve, reject) => {
    const query =
      "SELECT * FROM enrolled WHERE user_id = ? ORDER BY last_visited DESC LIMIT 1";
...
const getChapters = (courseId: number): Promise<chaptersType[]> => {
  return new Promise<chaptersType[]>((resolve, reject) => {
    const query =
      "SELECT chapter_id, chapter_title, chapter_number FROM chapters WHERE course_id = ? ORDER BY chapter_number ASC";
...
const queryCourses = (pagin: number, limit: number): Promise<courseType[]> => {
  return new Promise<courseType[]>((resolve, reject) => {
    const query =
      "SELECT course_id, course_name, course_title, course_description, created_at, status FROM  courses WHERE status != ? ORDER BY created_at DESC LIMIT  ? OFFSET  ?";
...
const queryCourseImage = (courseId: number) => {
  return new Promise<any>((resolve, reject) => {
    const query = "SELECT image FROM courses WHERE course_id = ?";
